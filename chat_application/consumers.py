...
class ChatroomConsumer(WebsocketConsumer):
    ...
    def message_handler(self,event):
        ...
        html = render_to_string("chat_application/partial/chat_message_p.html", context)

        # Send as JSON with hx-response header
    ...
    def online_count_handler(self,event):
        ...
        chat_messages=ChatGroup.objects.get(group_name='chat-meet').chat_messages.all()[:30]
        ...
        html=render_to_string("chat_application/partial/online_count.html",context)
        self.send(text_data=html)
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        public_chat_users = ChatGroup.objects.get(group_name='chat-meet').users_online.exclude(id=self.user.id)
        ...
        online_in_chats = (
            bool(public_chat_users) or
            bool(private_chats_with_users) or
            bool(group_chats_with_users)
        )
        ...
        html = render_to_string("chat_application/partial/online_status.html", context=context)
        self.send(text_data=html)
